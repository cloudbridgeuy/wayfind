...
use wayfind_v1_cli::sqlite::search::SqliteFts5Search;
use wayfind_v1_cli::sqlite::{row, SqliteStorage};
use wayfind_v1_core::{
...
fn ticket(path: &Path, id: i64) -> wayfind_v1_core::Ticket {
    let storage = SqliteStorage::open(path).expect("open the fixture");
    let parsed = storage
        .connection()
...
fn a_claim_row_missing_its_time_is_reported_as_corrupt() {
    ...
    assert!(
        matches!(error, wayfind_v1_core::StorageError::CorruptData(_)),
