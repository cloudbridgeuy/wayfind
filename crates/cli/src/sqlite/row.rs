...
fn timestamp(row: &Row<'_>, name: &str) -> StorageResult<Timestamp> {
    ...
    raw.parse().map_err(|error: wayfind_core::Error| {
...
fn corrupt(entity: &'static str, reason: impl Into<String>) -> StorageError {
    StorageError::CorruptData(wayfind_core::Error::corrupt_data(entity, reason))
}
...
fn parsed<T>(result: wayfind_core::Result<T>, name: &str) -> StorageResult<T> {
