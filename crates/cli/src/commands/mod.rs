...
//! `wayfind_core` what that means, apply whatever command the core returned,
...
impl Shell<'_> {
    ...
    pub fn map_document(&self, view: &InitiativeView) -> ShellResult<String> {
        ...
        for decision in &decisions {
            ...
            rows.push(wayfind_core::DecisionRow {
        ...
        let model = MapView {
            ...
            frontier: view
                ...
                .collect(),
            state: wayfind_core::classify_initiative(view),
            decisions: rows,
            fog: self
                .storage
        ...
        Ok(wayfind_core::render_map(&model))
    }
    ...
    pub fn ticket_document(&self, view: &InitiativeView, id: TicketId) -> ShellResult<String> {
        ...
        Ok(wayfind_core::render_ticket(&model))
    }
...
fn amended_at(ticket: &Ticket) -> Option<Timestamp> {
    match &ticket.state {
        wayfind_core::TicketState::Resolved { amended_at, .. } => *amended_at,
        _ => None,
    }
}
